            ...
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        ...
                        <x-ui.input id="code" type="text" name="code" class="block mt-1 w-1/2" inputmode="numeric" autofocus autocomplete="one-time-code"
                            wire:model.defer="code"
                            wire:keydown.enter="confirmTwoFactorAuthentication" />

                        <x-ui.input-error for="code" class="mt-2" />
                    </div>
                @endif
            @endif
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-ui.confirms-password wire:then="regenerateRecoveryCodes">
                        <x-ui.secondary-button class="mr-3">
                            {{ __('Regenerate Recovery Codes') }}
                        </x-ui.secondary-button>
                    </x-ui.confirms-password>
                @elseif ($showingConfirmation)
                    <x-ui.confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-ui.button type="button" class="mr-3" wire:loading.attr="disabled">
                            {{ __('Confirm') }}
                        </x-ui.button>
                    </x-ui.confirms-password>
                @else
